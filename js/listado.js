...
var restaurantABuscar = this.restaurantes.find(rest => rest.id === id);
    if (restaurantABuscar == undefined){
        return "No se ha encontrado ningún restaurant";
    }
    else {
        return restaurantABuscar
    }
...
/*      for (var i = 0; i < this.restaurantes.length; i++) {
    ...
    var ciudades2 = ciudades.filter(function (elem, index, self) {
        return index === self.indexOf(elem);
    });
*/
    ciudades = this.restaurantes.map(function(parametroRestaurante){
        return parametroRestaurante.ubicacion;
    });
    
    var horariosSinRepetir = eliminarRepetidos(ciudades);
...
/*     for (var i = 0; i < this.restaurantes.length; i++) {
    ...
    var rubro2 = rubro.filter(function (elem, index, self) {
        return index === self.indexOf(elem);
    });
*/

    Rubros = this.restaurantes.map(function(parametroRestaurante){
        return parametroRestaurante.rubro;
    });
    var horariosSinRepetir = eliminarRepetidos(Rubros);
    return horariosSinRepetir.sort();
}
...
/*    for (var i = 0; i < this.restaurantes.length; i++) {
    ...
    arregloHorarios.forEach(function (arreglo) {
        arreglo.forEach(function (horarios) {
            horarios.push(hor)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
         var horarios2 = h.filter(function(elem, index, self) {
         return index === self.indexOf(elem);
     });
    */ 

    Horarios = this.restaurantes.map(function(parametroRestaurante){
        return parametroRestaurante.Horarios;
    });

    var horariosSinRepetir = eliminarRepetidos(Horarios);
    return horariosSinRepetir.sort();
}

//Función que reemplaza la acción de eliminar los elementos repetidos de un arreglo
var eliminarRepetidos = function (repetido){
        var arrayFiltrado = repetido.filter(function (elem, index, self) {
            return index === self.indexOf(elem);
});
